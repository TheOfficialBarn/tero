...
export function PFP() {
    return (
        <>
            ...
            <div className="fixed top-0 left-0 w-full backdrop-filter backdrop-blur-md bg-white/0 dark:bg-black/0 z-10 pointer-events-none rounded-b-2xl h-[calc(env(safe-area-inset-top)+60px)] sm:h-[calc(env(safe-area-inset-top)+80px)]" />
            
            {/* Tero + PFP */}
            <section className="fixed top-12 sm:top-2 w-full px-4 flex justify-between items-center z-20">
                {/* Tero ☺️ */}
                <div className="
                    h-15
                    flex
                    ...
                    rounded-full
                ">
                    <Image src="/tero_192_2.png" alt="Logo" width={50} height={50} className="w-[50px] h-[50px] sm:w-[60px] sm:h-[60px]" priority />
                    <p className="bg-gradient-to-r from-yellow-600 via-orange-500 to-red-700 bg-clip-text text-transparent font-bold text-2xl sm:text-3xl">Tero</p>
                </div>
                {/* PFP 😮 */}
